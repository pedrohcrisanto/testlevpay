...
func getSuperHeros(w http.ResponseWriter, r *http.Request) {
	...
	for result.Next() {
		...
		err := result.Scan(&superhero.ID, &superhero.Name, &superhero.Biography.Fullname, &superhero.PowerStats.Intelligence, &superhero.PowerStats.Power,
			&superhero.Work.Occupation, &superhero.Image.Url, &superhero.UUID)
		if err != nil {
			panic(err.Error())
		}
